...
function Get-Args($p,[switch]$Batch){ $a=@(); if($Batch){$a+='-batch'}; $pw=Get-Secret ([string]$p.Id); $mode=[string]$p.AuthMode; if($mode -eq 'SavedSession'){if(-not $p.SavedSession){Fail 'Не указана PuTTY session.'};$si=Get-SavedSessionInfo ([string]$p.SavedSession);if(-not $si){Fail "PuTTY session '$([string]$p.SavedSession)' не найдена."};if(-not $pw -and -not(Test-SavedSessionNonPasswordAuth $si)){Fail 'SavedSession не имеет сохранённого DPAPI-пароля и не подтверждён .ppk/Pageant. Введите пароль один раз во вкладке VPS-серверы и сохраните профиль.'};$a+=@('-load',[string]$p.SavedSession);if($p.User){$a+=@('-l',[string]$p.User)};if($pw){$a+=@('-pw',$pw)}}else{if(-not $p.Host){Fail 'Не указан Host/IP.'};$a+=@('-ssh','-P',([string][int]$p.SshPort),'-l',[string]$p.User);if($mode -eq 'Password'){if($pw){$a+=@('-pw',$pw)}else{Fail 'Для режима IP/пароль сохранённый пароль не найден.'}}elseif($mode -eq 'PrivateKey'){if(-not $p.KeyFile -or -not(Test-Path -LiteralPath ([string]$p.KeyFile))){Fail 'Файл приватного ключа .ppk не найден.'};$a+=@('-i',[string]$p.KeyFile)}elseif($mode -eq 'Pageant'){$a+='-agent'}else{Fail "Неизвестный AuthMode: $mode"};$a+=[string]$p.Host}; return ,$a }
