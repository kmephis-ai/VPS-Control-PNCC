...
BASE_EVAL=ROOT/".pncc-dev/scripts/evaluate_reusable_autonomous_continuation_steady_state.py"
OP_EVAL=ROOT/".pncc-dev/scripts/evaluate_autonomous_continuation_human_by_exception_operationalization.py"
...
def validate_policy(p):
 ...
 expected={"EXECUTE_ONE_DELEGATED_TRANSACTION":"CONTINUE_UNDER_EXISTING_AUTHORITY_ONLY","READBACK_REQUIRED_BEFORE_NEXT_ITERATION":"READBACK_REQUIRED_BEFORE_NEXT_ITERATION","ITERATION_COMPLETE_NEXT_FRESH_ITERATION_ALLOWED":"NEXT_FRESH_ITERATION_ALLOWED","WAIT_ONLY":"WAIT_ONLY","STOP_ONLY":"STOP_ONLY","SEPARATE_AUTHORITY_REQUIRED":"SEPARATE_AUTHORITY_REQUIRED","BLOCKED":"BLOCKED"}
 if p.get("outcome_mapping")!=expected: raise HumanByExceptionSteadyStateError("OUTCOME_MAPPING_INVALID")
...
def validate_anchors(p,root=ROOT):
 ...
 e=load_json(path)
 txn=e.get("controlled_transaction",{})
...
def evaluate(snapshot,*,policy=None,root=ROOT,check_anchors=True):
 ...
 try:
  ...
  if ops.get("execution_admission_decision")!=base_snapshot.get("execution_admission_decision"): raise HumanByExceptionSteadyStateError("ADMISSION_OPERATIONALIZATION_BINDING_MISMATCH")
  basemod=_module(root/".pncc-dev/scripts/evaluate_reusable_autonomous_continuation_steady_state.py","pncc_base_steady_wu128")
  base=basemod.evaluate(base_snapshot,check_anchors=check_anchors)
  op=opmod.evaluate(ops,check_anchors=check_anchors)
  ...
  outcome=p["outcome_mapping"][b]
  return _out(outcome,p,base=base,op=op)
