...
for key in ('candidate_id','source_sha','artifact_filename','artifact_sha256','artifact_size_bytes'):
    expected = q['candidate'].get(key)
    if r.get(key) != expected: fail('candidate_identity_' + key)
...
serialized = json.dumps(r, sort_keys=True).lower()
for forbidden in ('\\users\\','localappdata','yandexdisk','dropbox','putty portable','password','dpapi','pid','hostname','username','89.','128.'):
    if forbidden in serialized: fail('forbidden_public_detail')
